...
async function balanceERC20(contract, holder, balance) {
    assert.equal(await contract.balanceOf(holder).valueOf(), balance, "erc20 balance is not equal");
}
...
async function sign(signer, types, data) {
    var h = abi.soliditySHA3(types, data),
        sig = web3.eth.sign(signer, h.toString('hex')).slice(2),
        r = `0x${sig.slice(0, 64)}`,
