...
contract('BWLottery', function (accounts) {
    ...
    it("set results; New game created automaticaly; " +
        ...
        "withdrowPrize  decreases reservedAmount; ",
        async function () {
        let activetime = new BigNumber(await lottery.activeGame.call()).valueOf();
        await management.registerContract(1, cashier.address);
        await management.registerContract(2, lottery.address);
        ...
        assert.equal(await results.gameRezervedPrizes.call(oldActiveGame,0), false, "claim is not equal")
        assert.equal(await results.gameRezervedPrizes.call(oldActiveGame,1), false, "claim is not equal")
        assert.equal(await results.gameRezervedPrizes.call(oldActiveGame,2), true, "claim is not equal")
        //2*10^18*0.8*0.05
        ...
        assert.equal(await results.collectedEthers.call().valueOf(), new BigNumber('1600000000000000000').sub("80000000000000000").valueOf(), "collectedEthers is not equal")
