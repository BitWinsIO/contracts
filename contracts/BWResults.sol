...
contract BWResults is BWManaged {
    ...
    function claim(uint256 _ticketId) public {
        ...
        require(
            lottery.prevGame() != 0
            && block.timestamp <= lottery.prevGame().add(TIME_TO_CHECK_TICKET),
            ERROR_ACCESS_DENIED
        );
        ...
        if (false == gameReservedPrizes[lottery.prevGame()][category.sub(1)]) {
            ...
            reservedAmount = reservedAmount.add(
                gameBalances[lottery.prevGame()].mul(management.payoutsPerCategory(category)).div(100)
            );
        }
    ...
    function withdrawPrize(uint256 _gameTimestampedId, uint256 _ticketId) public {
        ...
        require(
            ...
            && block.timestamp >= _gameTimestampedId.add(TIME_TO_CHECK_TICKET),
            ERROR_ACCESS_DENIED
        );
        require(
            block.timestamp <= _gameTimestampedId.add(TIME_TO_CHECK_TICKET).add(TIME_TO_CLAIM_PRIZE),
            ERROR_ACCESS_DENIED
        );
        ...
        uint256 value = gameBalances[_gameTimestampedId].mul(
            management.payoutsPerCategory(categoryId)
        ).div(100).div(winnersAmount);
